...
class WebPackGenerator {
  static async prepareFoldersAndFiles(visualPackage) {
    ...
    let precompileFolder = path.join(
    ...
    let packgeDropFolder = path.join(
      visualPackage.basePath,
      config.package.dropFolder
    );
    let visualPluginFile = path.join(
      visualPackage.basePath,
      config.build.precompileFolder,
      visualPlugin
    );
  ...
  async installAPIpackage() {
    let apiVersion = this.pbiviz.apiVersion
      ? `~${this.pbiviz.apiVersion}`
      : "latest";
    try {
      ConsoleWriter.info(`Installing API: ${apiVersion}...`);
      let { stdout, stderr } = await exec(
        `npm install --save powerbi-visuals-api@${apiVersion}`
      );
      ...
      if (
        ex.message.indexOf(
          "No matching version found for powerbi-visuals-api"
        ) !== -1
      ) {
        throw new Error(`Error: Invalid API version: ${apiVersion}`);
      }
      ConsoleWriter.error(
        `npm install powerbi-visuals-api@${apiVersion} failed`
      );
      return false;
    }
  }
  ...
  configureVisualPlugin(options, tsconfig, visualPackage) {
    const visualJSFilePath = visualPackage.buildPath(
      tsconfig.compilerOptions.out || tsconfig.compilerOptions.outDir
    );
    ...
    let visualPluginPath = path.join(
      process.cwd(),
      config.build.precompileFolder,
      visualPlugin
    );
  ...
  async configureCustomVisualsWebpackPlugin(visualPackage, options, tsconfig) {
    ...
    if (tsconfig.compilerOptions.outDir) {
      ...
      if (
        api === null ||
        (typeof this.pbiviz.apiVersion !== "undefined" &&
          this.pbiviz.apiVersion != api.version)
      ) {
      ...
      pluginConfiguration.schemaLocation = path.join(
        process.cwd(),
        ".api",
        "v" + this.pbiviz.apiVersion
      );
      pluginConfiguration.externalJS = [
        path.join(
          visualPackage.basePath,
          config.build.precompileFolder,
          "externalJS.js"
      ...
      pluginConfiguration.cssStyles = path.join(
        visualPackage.basePath,
        config.build.dropFolder,
        config.build.css
      );
    ...
    pluginConfiguration.customVisualID = `CustomVisual_${this.pbiviz.visual.guid}`.replace(
      /[^\w\s]/gi,
      ""
    );
    pluginConfiguration.devMode =
      typeof options.devMode === "undefined" ? true : options.devMode;
    ...
    pluginConfiguration.modules =
      typeof tsconfig.compilerOptions.outDir !== "undefined";
    pluginConfiguration.visualSourceLocation = path.posix
      .relative(config.build.precompileFolder, tsconfig.files[0])
      .replace(/(\.ts)x|\.ts/, "");
    pluginConfiguration.pluginLocation = path.join(
      config.build.precompileFolder,
      "visualPlugin.ts"
    );
  ...
  async appendPlugins(options, visualPackage, tsconfig) {
    ...
    let statsLocation = config.build.stats
      .split("/")
      .slice(0, -1)
      .join(path.sep);
